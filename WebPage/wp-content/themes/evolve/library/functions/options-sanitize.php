...
add_filter( 'evl_sanitize_typography_js', 'evl_sanitize_typography_js' );
...
// Numerical Values   
function evl_sanitize_numbers($input) {
    return intval($input); 
}

// Text Fields 

function evl_sanitize_text_field($input) {
  return sanitize_text_field( $input );
    
}

/*
function evl_sanitize_select($input) {
  return sanitize_text_field( $input );
    
}
*/

// Fontawesome Values Pending
function evl_sanitize_fontawesome($input) {
     return sanitize_text_field( $input );
}
// px/% values Pending 
function evl_sanitize_pixel($input) {
     return sanitize_text_field( $input );
    
}

// Select Choices 
function evl_sanitize_choices( $input, $setting ) {
    global $wp_customize;
   
  // get correct setting id 
    preg_match("^\[(.*)\]^",$setting->id,$settingidvalue);
    
    if(isset($settingidvalue[1]))
    {
      $correct_setting_id="evolve-theme_$settingidvalue[1]";
      $control = $wp_customize->get_control($correct_setting_id);
